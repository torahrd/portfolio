...
#### Phase 15-2-2: 店舗名検索UI改善（2025年6月完了）
- [x] 投稿作成画面での店舗名検索機能（部分一致候補・バッジ表示含む）を完全動作確認
  - [x] DB部分一致候補が正しく複数件表示されることを確認
  - [x] 「部分一致」バッジが正しく表示されることを確認
- **まとめ**: 店舗名検索の完全一致・前方一致・部分一致の優先度ロジック、UIバッジ表示まで完了。
