...
## 店舗検索機能の方針・ベストプラクティスまとめ（2025/07/07）

### 現状の実装方針
- 完全一致→前方一致→部分一致の優先度順で検索
- 各一致ごとに最大5件のみ取得し、優先度順で最大5件返す
- TextNormalizationServiceで正規化（ひらがな・カタカナ・ローマ字・略称等）も併用
- 1文字から検索可能（日本語1文字店舗名にも対応）
- LIMIT句でパフォーマンスを担保し、数万件規模まで十分実用的
- サジェスト・リアルタイム検索の世界的ベストプラクティスに準拠

### 今後の拡張・注意点
- データが10万件を超える場合や、より柔軟な検索（typo/ローマ字/英語/略称/あいまい検索）が必要な場合は全文検索エンジン（Meilisearch/Algolia/Elasticsearch等）＋Laravel Scoutの導入を検討
- 検索ロジックはサービス層にまとめておくことで将来的な移行が容易
- 店舗名の正規化・多言語対応はTextNormalizationServiceや別カラム活用で強化
- Google Places API連携時も同様の正規化・優先度ロジックを適用可能

### 備考
- 「渋谷」など地名単体でのヒットは現状不要
- 検索条件・ロジックは今後の運用・ユーザー要望に応じて柔軟に見直し

---

#### 次のアクション
1. 店舗選択時にGoogle Place IDで詳細情報を取得し、フォームに自動反映
2. 住所・電話番号・営業時間などの自動入力の動作確認
