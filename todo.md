...
- [x] 本番環境への反映完了（2025-08-21）

### 1.13 セキュリティ対策完了評価 ✅ 完了
...
- [x] セキュリティレベルの再評価 
  - SSL Labs: A+ 達成
  - Security Headers: A 達成
  - Mozilla Observatory: B (75/100) - 技術的制約により許容
- [x] Phase 1完了の確認 - 全セキュリティ対策実装完了
- [x] Phase 2の優先順位確認 - UI/UX改善とバグ修正を優先
...
### 2.3 「行きたいお店」保存機能MVP実装【最優先】
**目標**: 未訪問の店舗をウィッシュリストとして保存する機能
**作業時間**: 8時間
**TDD方式で実装**
...
##### 🔴 Red - 失敗するテスト作成
- [ ] tests/Feature/WishlistPostUITest.php作成
  - [ ] 未訪問ステータスがデフォルトで設定されるテスト
  - [ ] 訪問済み切り替え時のフィールド表示テスト

##### 🟢 Green - 最小実装
- [ ] 訪問ステータストグルボタンをフォーム上部に移動
- [ ] 非公開トグルボタンも上部に配置
- [ ] Alpine.jsでの動的フィールド制御実装
  - [ ] 未訪問時: 画像アップロード非表示
  - [ ] 未訪問時: 訪問日時・予算・リピートメニュー非表示
- [ ] デフォルトで未訪問（visited = false）に設定
- [ ] 写真追加と詳細情報の順序入れ替え
...
#### Phase 3: リンク機能実装（TDDサイクル3）
##### 🔴 Red - 失敗するテスト作成
- [ ] tests/Feature/PostLinkFeatureTest.php作成
  - [ ] リンク保存・表示のテスト
  - [ ] URLバリデーションテスト

##### 🟢 Green - 最小実装
- [ ] postsテーブルにlink_urlカラム追加（migration作成）
- [ ] Post.phpモデルにfillable追加
- [ ] 投稿フォームにリンク入力欄追加
- [ ] 投稿詳細でのリンク表示（自動リンク化）

##### 🟡 Refactor - 品質改善
- [ ] バリデーションルール追加（link_url）
- [ ] XSS対策確認
...
#### Phase 4: 訪問済み切り替え機能（TDDサイクル4）
##### 🔴 Red - 失敗するテスト作成
- [ ] tests/Feature/MarkAsVisitedTest.php作成
  - [ ] 「訪問済みにする」ボタンのテスト
  - [ ] 権限チェックテスト

##### 🟢 Green - 最小実装
- [ ] 投稿詳細に「訪問済みにする」ボタン追加（未訪問投稿のみ）
- [ ] クリックで編集画面へ遷移（visited=trueで開く）
- [ ] ルート追加: posts/{post}/mark-as-visited
...
#### ⚪ 最終記録
- [ ] MCPメモリに実装内容記録
- [ ] todo-archive.mdに完了タスク移動

### 2.4 店舗詳細ページSEO対策【法務確認済み】
**現状**: 認証必須でSEO効果なし
**目標**: 公開情報のみ検索エンジンに表示
**作業時間**: 3時間
**TDD方式で実装**
...
#### 🔴 Red - 失敗するテスト作成
- [ ] tests/Feature/PublicShopPageTest.php作成
  - [ ] 未認証でも店舗詳細が表示されるテスト
  - [ ] プライベート投稿が表示されないテスト
  - [ ] お気に入りボタンが認証要求するテスト

#### 🟢 Green - 最小実装
- [ ] routes/web.phpで店舗詳細を認証不要に変更
- [ ] ShopController::showで認証状態による処理分岐
- [ ] shops/show-public.blade.php作成（SEO最適化版）
- [ ] メタタグ・OGPタグ実装
- [ ] 構造化データ（JSON-LD）追加
...
### 2.8 Google Mapsピン表示問題の修正 ✅ 対処済み
**現状**: ピンの代わりに画像エラーアイコンが表示 → 解決済み
**影響**: 地図の視認性低下 → 解消
**作業時間**: 20分

### 2.9 ランディングページデザイン改善【保留】
**現状**: 設計書完成済み、実装未着手
