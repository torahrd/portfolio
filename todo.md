...
- [~] プロフィールリンク機能の実装（feature/profile-link-generation ブランチ）
  - [ ] 1. DB設計・マイグレーション確認
    ...
    - [ ] トークン生成時に「作成日時」を組み込む方式に修正（例: hash(user_id + ランダム値 + 作成日時)）
    - [ ] 有効期限は「3日（72時間）」で統一
  - [ ] 2. モデル・リレーション修正
    - [x] ProfileLinkモデル・UserモデルのprofileLinksリレーションを確認
    - [ ] トークン生成・有効期限・一意性ロジックの強化（作成日時組み込み）
    - [ ] 期限切れ・手動無効化時はDBから完全削除
  - [ ] 3. コントローラ・ルート設計
    - [ ] /profile-link/{token} ルート追加
    - [ ] 期限切れ時は「期限切れ」画面＋/postsリダイレクト
    - [ ] 有効期間中は再生成不可、手動無効化は可能（確認ダイアログ付き）
  - [ ] 4. Blade/UI実装
    - [ ] マイプロフィール画面（profile/partials/profile-card.blade.php）に「プロフィールリンク作成」ボタン追加（編集ボタンの上）
    - [ ] 有効期間中は「プロフィールリンク表示」ボタンに切り替え
    - [ ] モーダル（components/atoms/modal.blade.php）でリンク表示・有効期限・コピー・無効化ボタンを実装
    - [ ] 無効化ボタン押下時は確認ダイアログ→OKで完全削除
  - [ ] 5. コピー機能
    - [ ] Clipboard API＋execCommand＋手動コピー案内の実装
    - [ ] コピー失敗時は「手動でコピーしてください」と案内
  - [ ] 6. アクセス制御・認可
    - [ ] プライベートアカウントの場合はフォロー申請画面のみ表示、投稿は非表示
    - [ ] 有効期限・無効化後はトークン無効化
  - [ ] 7. テスト・動作確認
    - [ ] 有効期限・無効化・再利用リスク・UI/UXの動作確認
    - [ ] エラー・例外時のハンドリング
  - [ ] 8. ドキュメント・todo更新
    - [ ] 仕様・設計・運用ルールをtodo.mdに明記し、将来の再生成機能も「将来的タスク」に記載
