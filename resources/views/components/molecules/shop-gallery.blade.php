...
<div class="bg-white rounded-xl shadow-card p-6">
    ...
    <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3">
        ...
        <a href="{{ route('posts.show', $post) }}" class="aspect-square overflow-hidden rounded-lg group cursor-pointer bg-neutral-100 block">
            @if($post->image_url)
            <img src="{{ $post->image_url }}" alt="投稿画像" class="w-full h-full object-cover group-hover:scale-110 transition-transform duration-300" />
            @else
