...
<div x-data="shopSearch({ initialShop: @js($initialShop), mode: '{{ $mode }}' })" class="relative">
  ...
  <!-- 検索結果表示エリア -->
      <div x-show="shouldShowResults" class="absolute left-0 right-0 z-40 w-full bg-white border border-gray-200 rounded-md mt-1 shadow-lg">
    ...
    <div class="mt-4 p-4 bg-blue-50 rounded-md">
      <div class="flex items-center justify-between">
        <div>
          ...
          <div class="text-sm text-gray-600" x-show="shouldShowPhoneNumber" x-text="getPhoneNumberText"></div>
        </div>
  ...
  <!-- バリデーションUI（選択済みかつバリデーションOK時のみ） -->
      <div x-show="shouldShowValidSelection" class="mt-2 p-2 bg-green-50 border border-green-200 rounded flex items-center gap-2">
    <svg class="w-5 h-5 text-green-500" fill="currentColor" viewBox="0 0 20 20">
      <path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clip-rule="evenodd"></path>
    </svg>
    <span class="text-green-700 text-sm">店舗選択が有効です</span>
  </div>
      <div x-show="shouldShowInvalidSelection" class="mt-2 p-2 bg-red-50 border border-red-200 rounded flex items-center gap-2">
